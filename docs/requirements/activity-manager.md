...
| **Pet Health Service** | Platform service that provides the Pet’s latest weight for calorie estimation and receives exercise-amount **Health metric** sync from Activity writes/deletes. |
| **Recipient Owner** | Another Owner who receives an in-platform share of an Activity. |
| **Community feed** | Platform surface that accepts an Activity share reference for Community-visible shares. |
...
- Sync of exercise-amount **Health metric** to Pet Health on Activity create/update/delete (metric value = duration in minutes)
- In-platform share to another Owner and/or a Community; external share payload (link + summary); resolve and revoke shares
...
- Renaming **Exercise** in `CONTEXT.md` / Pet Health docs (follow-up domain-modeling)
- First-party WhatsApp / Telegram / Facebook / other vendor posting APIs
- Forum posts, Group activities, and Shared locations as share targets
...
- An Activity records: Pet id, Activity type, timestamp, **duration** (required), optional type-dependent **amount**, calories burned, optional media references, optional notes; GPS Activities also require route/GPS.
- Exercise-amount **Health metric** value for an Activity is **duration in minutes**.
...
- Shareable unit is a **single Activity**. Share snapshot includes: Pet display name, Activity type label, timestamp, duration, optional notes, calories burned, optional amount, media if present, and for GPS Activities the route (full route acceptable in this version).
...
- Must integrate with **Owner & Pet Manager** (ownership / Pet status) and **Pet Health** (latest weight; exercise-amount metric sync).
- Community in-platform share requires a Community feed (or equivalent) that accepts an Activity share reference; if unavailable, Community share fails closed or is feature-flagged without removing the requirement.
...
- Pet Health exposes (or will expose) APIs for latest weight and for writing/correcting/deleting exercise-amount Health metrics.
- A Community feed (or equivalent) can accept an Activity share reference for in-platform Community shares.
...
- Glossary alignment (**Exercise** → **Activity type**) is handled separately; this document already uses the new terms.
...
1. The Owner can create an Activity with Pet id, Activity type, timestamp, duration, and optional amount, notes, and media references.
...
3. Amount may be omitted; when present it follows type-dependent meaning (e.g. distance or count) without driving the Health metric in this version.
...
5. On success, an exercise-amount Health metric equal to duration in minutes is written to Pet Health for that Pet.
...
1. The Pet’s Owner can update permitted fields (timestamp, duration, amount, notes, media references, route for GPS Activities, Activity type when valid).
...
5. On success, the exercise-amount Health metric in Pet Health is corrected to match the new duration in minutes.
...
2. On success, the corresponding exercise-amount Health metric is removed or corrected in Pet Health.
...
### AM-FR-011 — Share Activity to a Community

**Description:** The Owner creates an in-platform share of one Activity to a Community feed.

**Acceptance criteria:**

1. The Pet’s Owner can share a single existing Activity to a Community that can accept a share reference.
2. On success, the Community feed can present the share under share-visibility rules.
3. Share fails if the caller is not the Pet’s Owner or the Activity does not exist.
4. Share fails closed (or is unavailable) if the Community feed cannot accept the share reference.
5. Forum posts, Group activities, and Shared locations are not valid targets in this version.
...
4. Community-visible shares are resolvable to callers authorized for that Community feed.
5. Resolved snapshots include: Pet display name, Activity type label, timestamp, duration, optional notes, calories burned, optional amount, media if present, and route for GPS Activities.
...
1. The Pet’s Owner can revoke an in-platform Owner share, a Community share, or an external share they created for that Activity.
...
1. An Owner cannot list, get, create, update, or delete Activities for a Pet they do not own, except by resolving a share addressed to them or a Community they may access.
...
**Description:** Latest weight for calorie estimation and exercise-amount Health metric persistence depend on Pet Health.
...
2. Activity create/update/delete syncs the exercise-amount Health metric (duration in minutes) to Pet Health.
