...
Every error from **Owner & Pet Manager**, **Pet Health Service**, **Activity Manager**, and **Authentication Service** uses [RFC 9457 Problem Details](https://www.rfc-editor.org/rfc/rfc9457.html) as `application/problem+json`. The body carries `type`, `title`, `status`, and `detail`. One Zod schema in `packages/contracts` defines that shape for client calls and for service-to-service calls.
