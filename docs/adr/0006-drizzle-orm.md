...
Each service queries its own PostgreSQL database with Drizzle ORM. Schema changes ship as drizzle-kit migrations owned by that service. There is no shared table schema across **Owner & Pet Manager**, **Pet Health Service**, **Activity Manager**, and **Authentication Service**.
